class GroupMemberListService < DestinyResource
  ...
  def self.find_all_members_data_object(group_id, membership_type = MembershipType::XBOX.id, page_no = 1)
  ...
  def self.find_ten_members(group_id, membership_type = MembershipType::XBOX.id, page_no = 1)
  ...
  def self.find_single(scope, options)
    ...
    path = self.site.to_s.sub(":group_id", scope[0].to_s.gsub(" ", "%20").downcase).sub(":membership_type", scope[2].to_s.gsub(" ", "%20").downcase).sub(":page_no", scope[1].to_s.gsub(" ", "%20").downcase).sub("http://www.bungie.net", "")
    #raise "path: #{path}"
